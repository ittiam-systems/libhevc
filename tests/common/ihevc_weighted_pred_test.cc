...
class WeightedPredTestBase
 ...
 protected:
  void SetUp() override {
    ...
    src_strd = kComponents * wd * src_stride_mul;
    dst_strd = kComponents * wd * dst_stride_mul;

    src_buf1.resize(src_strd * ht + 16);
    src_buf2.resize(src_strd * ht + 16);
    dst_buf_ref.resize(dst_strd * ht + 16, 0xAA);
    dst_buf_tst.resize(dst_strd * ht + 16, 0xAA);

    FillRandom(src_buf1, static_cast<WORD16>(-8192), static_cast<WORD16>(8191),
               42);
    FillRandom(src_buf2, static_cast<WORD16>(-8192), static_cast<WORD16>(8191),
               43);
...
class WeightedPredUniLumaTest : public WeightedPredTestBase<1> {};
...
TEST_P(WeightedPredUniLumaTest, Run) {
  ...
  ref->ihevc_weighted_pred_uni_fptr(src_buf1.data(), dst_buf_ref.data(),
                                    src_strd, dst_strd, wgt0, off0, shift,
                                    lvl_shift, ht, wd);

  tst->ihevc_weighted_pred_uni_fptr(src_buf1.data(), dst_buf_tst.data(),
                                    src_strd, dst_strd, wgt0, off0, shift,
                                    lvl_shift, ht, wd);
...
class WeightedPredUniChromaTest : public WeightedPredTestBase<2> {};
...
TEST_P(WeightedPredUniChromaTest, Run) {
  ...
  ref->ihevc_weighted_pred_chroma_uni_fptr(
      src_buf1.data(), dst_buf_ref.data(), src_strd, dst_strd, wgt0_cb, wgt0_cr,
      off0_cb, off0_cr, shift, lvl_shift, ht, wd);

  tst->ihevc_weighted_pred_chroma_uni_fptr(
      src_buf1.data(), dst_buf_tst.data(), src_strd, dst_strd, wgt0_cb, wgt0_cr,
      off0_cb, off0_cr, shift, lvl_shift, ht, wd);
...
class WeightedPredBiLumaTest : public WeightedPredTestBase<1> {};
...
class WeightedPredBiChromaTest : public WeightedPredTestBase<2> {};
...
class WeightedPredBiDefaultLumaTest : public WeightedPredTestBase<1> {};
...
class WeightedPredBiDefaultChromaTest : public WeightedPredTestBase<2> {};
