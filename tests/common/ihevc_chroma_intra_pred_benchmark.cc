...
void BM_ChromaIntraPred(benchmark::State& state, IntraPredConfig config) {
  ...
  ChromaIntraPredFn fn = GetChromaIntraPredFn(selector, mode);
  if (!fn) {
    state.SkipWithError("Target function pointer is null");
    return;
  }

  IntraPredChromaBuffers<UWORD8> buf =
      CreateIntraPredChromaBuffers<UWORD8>(nt, config.dst_strd_mul);

  // Correctness verification against C reference prior to measurement
  if (arch != ARCH_NA) {
    ChromaIntraPredFn ref_fn = GetChromaIntraPredFn(get_ref_func_ptr(), mode);
    if (ref_fn) {
      IntraPredChromaBuffers<UWORD8> ref_buf = buf;
      IntraPredChromaBuffers<UWORD8> tst_buf = buf;
  ...
  state.SetBytesProcessed(state.iterations() * 2 * nt * nt * sizeof(UWORD8));
}
...
void RegisterAllBenchmarks() {
  ...
  // Register Chroma Intra Prediction benchmarks
  for (const auto& mode_info : GetIntraPredBenchmarkModes()) {
    for (int nt : GetIntraPredBenchmarkSizes()) {
      for (auto arch : arches) {
        ...
        if (!sel || !GetChromaIntraPredFn(sel, mode_info.mode)) {
          continue;
        }
        std::string arch_name = GetArchName(arch);
        std::string name = "BM_ChromaIntraPred/" + std::string(mode_info.name) +
