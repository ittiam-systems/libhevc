                                        ...
                                        WORD32 log2_cb_size)
{
    ...
    for(i = 0; i < luma_samples; i++)
    {
        ...
        if (i4_bit_depth > 8)
        {
            *(UWORD16 *)pu1_coeff_data = value << (i4_bit_depth - num_bits);
            pu1_coeff_data += 2;
        }
        else
        {
...
IHEVCD_ERROR_T ihevcd_parse_slice_data(codec_t *ps_codec)
{
    ...
    do
    {
                ...
                (1 == ps_slice_hdr->i1_slice_disable_deblocking_filter_flag))
        {
            ...
            pi1_qp = ps_bs_ctxt->pi1_pic_qp + ((ps_codec->s_parse.i4_ctb_x + ps_codec->s_parse.i4_ctb_y * qp_strd) << (log2_ctb_size - 3));
