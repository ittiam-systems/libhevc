...
*
* @brief
*  Contains function Definition for intra prediction  interpolation filters for higher bit depths
*
*
* @author
*  Ittiam
                                    ...
                                    UWORD8 bit_depth)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for(i = 0; i < (four_nt - 1); i++)
            {
                au2_flt[i + 1] = (pu2_src[i] + 2 * pu2_src[i + 1]
                                + pu2_src[i + 2] + 2) >> 2;
            }
        }
                                  ...
                                  UWORD8 bit_depth)
{
    ...
    for(row = 0; row < nt; row++)
    {
        ...
        {
            pu2_dst[row * dst_strd + col] = ((nt - 1 - col)
                            * pu2_ref[two_nt - 1 - row]
                            + (col + 1) * pu2_ref[three_nt + 1]
                            + (nt - 1 - row) * pu2_ref[two_nt + 1 + col]
                            + (row + 1) * pu2_ref[nt - 1] + nt) >> (log2nt + 1);
        }
    }
}
                              ...
                              UWORD8 bit_depth)
{
    ...
    else
    {
        ...
        for(col = 1; col < nt; col++)
            pu2_dst[col] = (pu2_ref[two_nt + 1 + col] + three_dc_val + 2) >> 2;
                                ...
                                UWORD8 bit_depth)
{
    ...
    else
    {
        ...
        for(col = 0; col < nt; col++)
        {
            ...
            pu2_dst[col] = CLIP3(s2_predpixel,0,((1<<bit_depth)-1));
                               ...
                               UWORD8 bit_depth)
{
    ...
    for(col = 0; col < nt; col++)
    {
        ...
        for(row = 0; row < nt; row++)
            pu2_dst[col + (row * dst_strd)] = pu2_ref[two_nt - row - idx - 1];
    }
}
...
#endif
        for(col = 0; col < nt; col++)
            pu2_dst[col + (row * dst_strd)] = pu2_ref[two_nt + col + idx + 1];
    }
}
    ...
    for(col = 0; col < nt; col++)
    {
        ...
        for(row = 0; row < nt; row++)
        {
            ...
            pu2_dst[col + (row * dst_strd)] = (((32 - fract)
                            * pu2_ref[ref_main_idx]
                            + fract * pu2_ref[ref_main_idx - 1] + 16) >> 5 );
        }
    }
}
                                         ...
                                         UWORD8 bit_depth)
{
    ...
    for(col = 0; col < nt; col++)
    {
        ...
        for(row = 0; row < nt; row++)
        {
            ...
            pu2_dst[col + (dst_strd * row)] = (UWORD16)(((32 - fract)
