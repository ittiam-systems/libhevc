                                       ...
                                       UWORD8 u1_bit_depth)
{
    ...
    for(j = 0; j < trans_size; j++)
    {
        ...
        itrans_out =
                        CLIP_S16( ((e[0] + o[0] + add)>>shift ) );
        pu2_dst[0 * 2] = CLIP3( (itrans_out + pu2_pred[0 * 2] ), 0, clip_limit);
        itrans_out =
                        CLIP_S16( ((e[1] + o[1] + add)>>shift ) );
        pu2_dst[1 * 2] = CLIP3( (itrans_out + pu2_pred[1 * 2] ), 0, clip_limit);
        itrans_out =
                        CLIP_S16( ((e[1] - o[1] + add)>>shift ) );
        pu2_dst[2 * 2] = CLIP3( (itrans_out + pu2_pred[2 * 2] ), 0, clip_limit);
        itrans_out =
                        CLIP_S16( ((e[0] - o[0] + add)>>shift ));
