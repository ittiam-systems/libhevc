...
*******************************************************************************
* @file
*  ihevc_hbd_chroma_intra_pred_filters.c
*
* @brief
                                                  ...
                                                  UWORD8 bit_depth)
{
    ...
    else
    {
        ...
        if(0 != tp_left)
        {
            pu2_dst[(4 * nt )] = *pu2_top_left ; // U top-left sample
            pu2_dst[(4 * nt ) + 1] = *(pu2_top_left + 1 ) ; // V top-left sample
        }
        /* Left nbrs  */
        if(0 != left)
        {
            ...
            {
                pu2_dst[(4 * nt ) - 2 - i] = pu2_left[j * src_strd]; // U left samples
                pu2_dst[(4 * nt ) - 1 - i] = pu2_left[(j * src_strd) + 1]; // V left samples
                j++;
            }
        }
        /* Bottom - Left nbrs  */
        ...
        if(nt == 4 || (nt == 8 && chroma_format_idc == CHROMA_FMT_IDC_YUV444))
        {
            ...
            if(left == 0)
            {
                for(i = 0; i < (2 * nt); i+=2)
                {
                    pu2_dst[(2 * nt) + i] = pu2_dst[(2 * nt) - 2];
                    pu2_dst[(2 * nt) + 1 + i] = pu2_dst[(2 * nt) - 1];
                }
            }
            if(tp_left == 0)
            {
                pu2_dst[4 * nt] = pu2_dst[(4 * nt) - 2];
                pu2_dst[(4 * nt) + 1] = pu2_dst[(4 * nt) - 1];
            }
            if(top == 0)
            {
                for(i = 0; i < (2 * nt); i+=2)
                {
                    pu2_dst[(4 * nt) + 2 + i] = pu2_dst[(4 * nt)];
                    pu2_dst[(4 * nt) + 3 + i] = pu2_dst[(4 * nt) + 1];
                }
            }
            if(tp_right == 0)
            {
                for(i = 0; i < (2 * nt); i+=2)
                {
                    pu2_dst[(6 * nt) + 2 + i] = pu2_dst[(6 * nt)];
                    pu2_dst[(6 * nt) + 3 + i] = pu2_dst[(6 * nt) + 1];
                }
            }
        }
                                    ...
                                    WORD32 mode)
{
    ...
    for(row = 0; row < nt; row++)
    {
        for(col = 0; col < (2 * nt); col+=2)
        {
            pu2_dst[(row * dst_strd) + col] = dc_val_u;
            pu2_dst[(row * dst_strd) + col + 1] = dc_val_v;
        }
    }
}
                                      ...
                                      WORD32 mode)
{
    ...
    for(row = 0; row < nt ; row++)
    {
        for(col = 0; col < (2 * nt); col+=2)
        {
            pu2_dst[(row * dst_strd) + col] = pu2_ref[(4 * nt) - 2 - 2 * row];
            pu2_dst[(row * dst_strd) + col+1] = pu2_ref[(4 * nt) - 1 - 2 * row];
        }
    }
}
                                     ...
                                     WORD32 mode)
{
    ...
    for(row = 0; row < nt; row++)
    {
        for(col =0; col < (2 * nt); col+=2)
        {
            pu2_dst[(row * dst_strd) + col] = pu2_ref[(4 * nt) + 2 + col];
            pu2_dst[(row * dst_strd) + col+1] = pu2_ref[(4 * nt) + 3 + col];
        }
    }
}
                                       ...
                                       WORD32 mode)
{
    ...
    for(col = 0; col < (2 * nt); col+=2)
    {
        ...
        for(row = 0; row < nt; row++)
        {
            pu2_dst[col + (row * dst_strd)] = pu2_ref[(4 * nt) - 2 * row - idx_u - 3];
            pu2_dst[(col + 1) + (row * dst_strd)] = pu2_ref[(4 * nt) - 2 * row - idx_v - 1];
        }
    }
}
                                            ...
                                            WORD32 mode)
{
    ...
    for(row = 0; row < nt; row++)
    {
        ...
        for(col = 0; col < (2 * nt); col+=2)
        {
            pu2_dst[col + (row * dst_strd)] = pu2_ref[(4 * nt) + col + 2 * idx + 2];
            pu2_dst[(col + 1) + (row * dst_strd)] = pu2_ref[(4 * nt) + (col + 1) + 2 * idx + 2];
        }
    }
}
                                             ...
                                             WORD32 mode)
{
    ...
    for(col = 0; col < (2 * nt); col += 2)
    {
        ...
        for(row = 0; row < nt; row++)
        {
            ...
            pu2_dst[col + (row * dst_strd)] = (((32 - fract_u)
                            * pu2_ref[ref_main_idx_u]
                            + fract_u * pu2_ref[ref_main_idx_u - 2] + 16) >> 5 );

            pu2_dst[(col + 1) + (row * dst_strd)] = (((32 - fract_v)
                            * pu2_ref[ref_main_idx_v]
                            + fract_v * pu2_ref[ref_main_idx_v - 2] + 16) >> 5 );
        }
    }
}
                                               ...
                                               WORD32 mode)
{
    ...
    for(col = 0; col < (2 * nt); col+=2)
    {
        ...
        for(row = 0; row < nt; row++)
        {
            ...
            pu2_dst[col + (dst_strd * row)] = (UWORD16)(((32 - fract_u)
                            * ref_main[ref_main_idx_u]
                            + fract_u * ref_main[ref_main_idx_u + 2] + 16) >> 5 );
            pu2_dst[(col + 1) + (dst_strd * row)] = (UWORD16)(((32 - fract_v)
                                               ...
                                               WORD32 mode)
{
    ...
    for(row = 0; row < nt; row++)
    {
        ...
        for(col = 0; col < (2 * nt); col+=2)
        {
            ...
            pu2_dst[(row * dst_strd) + col] = (UWORD16)(((32 - fract)
                            * ref_main[ref_main_idx_u]
                            + fract * ref_main[ref_main_idx_u + 2] + 16) >> 5 );
            pu2_dst[(row * dst_strd) + (col + 1)] = (UWORD16)(((32 - fract)
                                        * ref_main[ref_main_idx_v]
                                               ...
                                               WORD32 mode)
{
    ...
    for(row = 0; row < nt; row++)
    {
        ...
        for(col = 0; col < (2 * nt); col+=2)
        {
            ...
            pu2_dst[col + (row * dst_strd)] = (((32 - fract)
                            * pu2_ref[ref_main_idx_u]
                            + fract * pu2_ref[ref_main_idx_u + 2] + 16) >> 5 );
            pu2_dst[(col+1) + (row * dst_strd)] = (((32 - fract)
                                        * pu2_ref[ref_main_idx_v]
                                        + fract * pu2_ref[ref_main_idx_v + 2] + 16) >> 5 );
        }
    }
}
