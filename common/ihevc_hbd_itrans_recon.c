                                   ...
                                   UWORD8 u1_bit_depth)
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        ...
        itrans_out =
                        CLIP_S16(( 29 * c[0] + 55 * c[1] + c[3] + add ) >> shift );
        pu2_dst[0] = CLIP3( (itrans_out + pu2_pred[0] ), 0, clip_limit );
        itrans_out =
                        CLIP_S16(( 55 * c[2] - 29 * c[1] + c[3] + add ) >> shift );
        pu2_dst[1] = CLIP3( (itrans_out + pu2_pred[1] ), 0, clip_limit );
        itrans_out =
                        CLIP_S16(( 74 * (pi2_tmp[0] - pi2_tmp[2 * trans_size] + pi2_tmp[3 * trans_size]) + add ) >> shift );
        pu2_dst[2] = CLIP3( (itrans_out+ pu2_pred[2] ), 0, clip_limit );
        itrans_out =
                        CLIP_S16(( 55 * c[0] + 29 * c[2] - c[3] + add ) >> shift );
